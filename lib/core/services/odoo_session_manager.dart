...
class OdooSessionManager {
  ...
  static bool _isAuthError(Object e) {
    ...
    return errorStr.contains('401') ||
        ...
        (errorStr.contains('odooexception') &&
  ...
  static Future<AppSessionData?> getCurrentSession() async {
    ...
    try {
      ...
      final client = OdooClient(saved.serverUrl, sessionId: saved.odooSession);
      _client = client;
      ...
      try {
        ...
        final info = await client.callRPC(
          '/web/session/get_session_info',
          'call',
          {},
        );

        if (info != null && info['uid'] != null && info['uid'] != false) {
          _client = client;

          final newVersion = info['server_version']?.toString() ?? '';
          if (newVersion.isNotEmpty &&
              newVersion != saved.odooSession.serverVersion) {
            final updatedOdooSession = OdooSession(
              id: saved.odooSession.id,
              userId: saved.odooSession.userId,
              partnerId: saved.odooSession.partnerId,
              companyId: saved.odooSession.companyId,
              allowedCompanies: saved.odooSession.allowedCompanies,
              userLogin: saved.odooSession.userLogin,
              userName: saved.odooSession.userName,
              userLang: saved.odooSession.userLang,
              userTz: saved.odooSession.userTz,
              isSystem: saved.odooSession.isSystem,
              dbName: saved.odooSession.dbName,
              serverVersion: newVersion,
            );

            final updatedSession = saved.copyWith(
              odooSession: updatedOdooSession,
            );
  ...
  static Future<bool> refreshSession() async {
    ...
    if (session == null || session.userLogin.isEmpty) {
      return false;
    }
  ...
  static Future<OdooClient> getClientEnsured() async {
    ...
    if (_client != null &&
        _cachedSession != null &&
        _client!.sessionId?.userId == _cachedSession!.userId &&
        _lastAuthTime != null &&
  ...
  static Future<void> saveSessionFromExistingSession(
    ...
    await sessionService.storeAccount(sessionData, password);

    /// Switch to the newly added account
    await sessionService.switchToAccount(sessionData);
