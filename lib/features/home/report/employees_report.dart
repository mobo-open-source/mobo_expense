...
class EmployeesReport extends StatefulWidget {
  final List<dynamic> companyEmployeeReport;
  const EmployeesReport({super.key, required this.companyEmployeeReport});
...
class _EmployeesReportState extends State<EmployeesReport> {
  ...
  Widget build(BuildContext context) {
    ...
    /// Convert employee data once
    final employeeData = widget.companyEmployeeReport
        .map(
          (item) => MonthlyAmountModel(
            month: item['employee_id'][1],
                    ...
                    onChanged: (value) async {
                      ...
                      if (value == ReportAction.download) {
                        if (employeeData.isNotEmpty) {
                          await createExcel(employeeData);
                        }
                        return;
                      }
                    ...
                    child: simpleCardWidget(
                      Column(
                        ...
                        children: [
                          ...
                          PieChartWidget(data: employeeData),
                        ],
                      ),
                    ),
                  ),
