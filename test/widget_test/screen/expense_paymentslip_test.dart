...
void main() {
  ...
  testWidgets("payment slip loading", (WidgetTester tester) async {
    when(() => expenseProvider.invoiceLoading).thenReturn(true);
    when(
      () => expenseProvider.gettingInvoice(any(), any()),
    ).thenAnswer((_) async {});
    await tester.pumpWidget(
      wrapWithProviders(
        child: PaymentSlipPage(id: 1),
  ...
  testWidgets("payment slip page  with no payment slip", (
    WidgetTester tester,
  ) async {
    ...
    expect(find.text("No payment data found"), findsOneWidget);
  });

  testWidgets("show payment slip", (WidgetTester tester) async {
    when(() => expenseProvider.invoiceLoading).thenReturn(false);
    when(
      () => expenseProvider.gettingInvoice(any(), any()),
    ).thenAnswer((_) async {});
    when(() => expenseProvider.paySlip).thenReturn({"name": "sample"});
    ...
    expect(find.text("No payment data found"), findsNothing);
